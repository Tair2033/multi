...
token = '1222570218:AAFk2qrWpRorqmJdBjcXZr2oHpRc8qLY9TY'
...
def main(message):
    ...
    key.row("Covid", "Узнать факт","/menu")
    key.row("Крестики-нолики(не сделал)")
    btn=bot.send_message(message.chat.id, "Напиши мне сучка, ДАВАЙ!" ,reply_markup=key )
    bot.register_next_step_handler(btn, choice)

def choice(message):
    lok=random.randint(0, 6)
    if message.text == "Covid":
        bot.send_message(message.chat.id, str(date) +"\n"+"Заболевших в Самаре за день: " + newinfected[0].text + "\n" + "Всего Заболевших в Самаре: " + allpeopleinfected[0].text + "\n" + "Всего заболевший в мире: " + allworld[0].text + 2*"\n" + "Температура сегодня: " + temp[0].get_text("|", strip=True))
    if message.text == "Узнать факт":
        num=random.randint(0, len(facts))
        bot.send_message(message.chat.id, facts[num])
...
def send_fact_message():
    num=random.randint(0, len(facts))
    bot.send_message(-1001403887404, "Ночной факт: " + facts[num])
schedule.every().day.at("09:00").do(send_covid_message)
schedule.every().day.at("22:05").do(send_fact_message)
while True:
